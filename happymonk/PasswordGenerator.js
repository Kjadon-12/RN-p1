...
import {Text,View,Button, ScrollView,SafeAreaView,StyleSheet,TextInput,TouchableOpacity} from 'react-native'
...
const PasswordGenerator = ()=>{
   ...
   const generatePasswordString = (passwordLength)=>{
    ...
    let numCharacter = '0123456789'
    let specialCharacter = '!@#$%^&*()_+'

    if(lowerCase){
        charList += lowerCaseCharacter;
    }
    if(upperCase){
        charList += upperCaseCharacter;
    }
    if(numbers){
        charList += numCharacter;
    }
    if(symbols){
...
const createPassword = (characters,passwordLength)=>{
    ...
    for(let i=0;i<passwordLength;i++){
        const charIndex = Math.round(Math.random()* characters.length)
         res += characters.charAt(charIndex)
    }
